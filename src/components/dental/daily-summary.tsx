...
import {
  ...
  ChevronLeft, ChevronRight, CalendarDays, User, CreditCard
} from 'lucide-react';
  ...
  const fetchData = async (date: string) => {
    ...
    try {
      ...
      if (paymentsRes.ok) {
        ...
        // Filter to only payments on this date
      ...
      if (proceduresRes.ok) {
        ...
        // Filter procedures created on this date
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="bg-emerald-50 border-emerald-200">
          <CardContent className="p-5">
            ...
            <p className="text-3xl font-bold text-emerald-700">${totalIncome.toFixed(2)}</p>
        ...
        <Card className="bg-purple-50 border-purple-200">
          <CardContent className="p-5">
            ...
            <p className="text-3xl font-bold text-purple-700">${totalLabExpenses.toFixed(2)}</p>
            ...
            {payments.length > 0 ? (
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center px-1">
                  ...
                  <span className="font-bold text-emerald-700">${totalIncome.toFixed(2)}</span>
                </div>
              </div>
            ) : (
                ...
                {Object.entries(labExpensesByPatient).map(([patientName, data]) => (
                  <div key={patientName} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    <div className="flex items-center justify-between p-3 bg-purple-50/50 border-b border-gray-100">
                      ...
                      <span className="font-bold text-purple-700">${data.total.toFixed(2)}</span>
                    </div>
            ...
            <div className="space-y-2">
              {procedures.map((tp) => (
                <div key={tp.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-gray-900 text-sm">${tp.procedure?.price.toFixed(2)}</span>
                    {tp.completed ? (
